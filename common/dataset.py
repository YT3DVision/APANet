...
class MyTrainDataSet(Dataset):
    def __init__(self, root_path, patch_size=128, length=None):
        super(MyTrainDataSet, self).__init__()
        inputPathTrainLeft = root_path + "/rain_left"
        inputPathTrainRight = root_path + "/rain_right"
        targetPathTrainLeft = root_path + "/gt_left"
        targetPathTrainRight = root_path + "/gt_right"
    ...
    def __len__(self):
        return len(self.targetImagesLeft) if self.length is None else self.length
...
class RealDataSet(Dataset):
    def __init__(self, root_path, patch_size=64, length=0):
        super(RealDataSet, self).__init__()
        inputPathTrainLeft = root_path + "/left"
        inputPathTrainRight = root_path + "/right"
        self.inputPathLeft = inputPathTrainLeft
        self.inputImagesLeft = os.listdir(inputPathTrainLeft)
        self.inputImagesLeft.sort(key=lambda x: (x.split('.')[0]))
        self.inputPathRight = inputPathTrainRight
    ...
    def __getitem__(self, index):
        ...
        hh, ww = inputImageLeft.shape[1], inputImageLeft.shape[2]
